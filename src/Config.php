...
class Config
{
	...
	public const META_DATA_NS = 'urn:oasis:names:tc:SAML:2.0:metadata';
	public const DIGITAL_SIGNATURE_NS = 'http://www.w3.org/2000/09/xmldsig#';
	...
	{
		$aSP = MetaModel::GetModuleSetting('combodo-saml', 'sp', []);
		$aIDP = MetaModel::GetModuleSetting('combodo-saml', 'idp', []);
		$aSecurity = MetaModel::GetModuleSetting('combodo-saml', 'security', []);
	...
	public static function GetSPSettings()
	{
		...
		if (isset($aConfSettings['sp']['x509cert'])) {
			$aSP['key'] = str_replace(['-----END CERTIFICATE-----', '-----BEGIN CERTIFICATE-----', "\n", "\r"], '', $aConfSettings['sp']['x509cert']);
		}
		return $aSP;
	}
	...
	public static function FillSPSettings(&$aSP)
	{
		...
		$aSP['singleLogoutService'] = [
				'binding' => static::BINDING_HTTP_REDIRECT,
				'url' => $sPath.'/sls.php',
			];
		$aSP['assertionConsumerService'] = [
				'binding' => static::BINDING_HTTP_POST,
				'url' => $sPath.'/acs.php',
			];
		if (isset($aConfSettings['sp']['x509cert'])) {
